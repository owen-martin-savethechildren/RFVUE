...
function MenuEditor(idSelector, options) {
    ...
    function editItem($item) {
        ...
        $.each(data, function (p, v) {
            //console.log(p);
            var type = $form.find("[name=" + p + "]").attr('type');
            if (type == 'checkbox') {
                $form.find("[name=" + p + "]").prop("checked", v);
            }
            else
    ...
    function TButtonGroup() {
        ...
        var $divbtn = $('<div>').addClass('btn-group float-right setting-btn-group');
        var $btnEdit = TButton({ classCss: 'btn btn-primary  btn-sm btn-sm-setting btnEdit', text: settings.labelEdit });
        var $btnRemv = TButton({ classCss: 'btn btn-danger btn-sm btn-sm-setting btnRemove', text: settings.labelRemove });
        var $btnUp = TButton({ classCss: 'btn btn-secondary btn-sm btn-sm-setting btnUp btnMove', text: '<i class="fas fa-angle-up clickable"></i>' });
        var $btnDown = TButton({ classCss: 'btn btn-secondary btn-sm btn-sm-setting btnDown btnMove', text: '<i class="fas fa-angle-down clickable"></i>' });
        var $btnOut = TButton({ classCss: 'btn btn-secondary btn-sm btn-sm-setting btnOut btnMove', text: '<i class="fas fa-level-down-alt clickable"></i>' });
        var $btnIn = TButton({ classCss: 'btn btn-secondary btn-sm btn-sm-setting btnIn btnMove', text: '<i class="fas fa-level-up-alt clickable"></i>' });
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
           // debugger;
            var IsModel = (v.IsModel === true) ? 'model' : ''; 
            ...
            var $span = $('<span>').addClass('txt').append(v.DisplayName).css('margin-right', '5px');
            var $modelOrMenu = $('<span>').addClass('txt is-model badge badge-pill badge-info').append(IsModel).css('margin-left', '10px');
            var $divbtn = TButtonGroup();
            $div.append($i).append("&nbsp;").append($span).append($modelOrMenu).append($divbtn);
            $li.append($div);
            if (isParent) {
                $li.addClass('parent');
    ...
    function TOpener(li){
        var opener = $('<span>').addClass('sortableListsOpener ' + options.opener.openerClass).css(options.opener.openerCss)
                .on('mousedown touchstart', function (e){
                    var li = $(this).closest('li');
                    if (li.hasClass('sortableListsClosed')) { //sortableListsOpen //sortableListsClosed
                        li.iconOpen(options);
                    } else {
        ...
        if (!li.hasClass('sortableListsOpen')) {  //sortableListsOpen
            li.iconClose(options);
        } else {
            li.iconOpen(options);
        }
