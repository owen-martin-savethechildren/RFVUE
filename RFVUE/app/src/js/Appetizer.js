...
var appetizer = {
    ...
    tools: {
        ...
        resetPage: function (divNames, kendoGrids, kendoMultis, kendoListViews, kendoTree, kendoUpload, focusControl) {
            ...
            $('#' + divNames + ' input:radio:checked').each(function () {
                $(this).attr('checked', false);
            });

            ///reset all checkbox
            $('#' + divNames + ' input:checkbox:checked').each(function () {
                $(this).attr('checked', false);
            });

            ///reset all kendo text editor
            $('#' + divNames + ' textarea').each(function () {
    ...
    div: {
        ...
        getJsonObjectOnlyFromDiv: function (divId) {
            var jo = new appetizer.jsonObject();
            $("#" + divId + " :input:not([type=button]):not([type=submit]):not([type=reset])").each(function () {
                if ($(this).attr('data-role') == 'dropdownlist') {
                    if ($(this).attr('data-role') !== 'undefined') {
                        jo.add($(this).attr('id'), $(this).val());
                    }
                }
                else if ($(this).attr('data-role') == 'multiselect') {
                    jo.add($(this).attr('id'), $(this).data("kendoMultiSelect").value().join());
                }
                else {
        ...
        setJsonObjectToDiv: function (jsonVal, primaryKeyTag) {
            //;
            var jObj = jsonVal;// JSON.parse(jsonString);
            var nodeDiv = document.getElementById(primaryKeyTag);
            ...
            for (var key in jObj) {
                ...
                if (key != "") {
                    ...
                    var fieldName = key;
                    var cellValue = (jObj[key]);
                    var node = $("#" + primaryKeyTag).find("#" + key);//"#"+primaryKeyTag > 
                    // var nodeName = $("#" + primaryKeyTag).find("#" + key);
                    var nodeName = key;
                    ...
                    if (fieldName == primaryKeyTag) {
                        ...
                        // 
                        if (isDropDownList() == 'k-dropdown-wrap k-state-default') {
                            var dropdownlist = $("#" + primaryKeyTag).find("#" + fieldName).data("kendoDropDownList");
                            dropdownlist.value(jObj[fieldName.replace('Name', 'ID')]);
                        }
                        else if (isDateTimePicker() == "k-widget k-datetimepicker k-header") {
                            var datepicker = $("#" + primaryKeyTag).find('#' + fieldName).data("kendoDateTimePicker");
                            var dateVal = new Date(cellValue);
                            datepicker.value(dateVal);
                        }
                        else if (isDateTimePicker() == "k-widget k-datepicker k-header") {
                            var datepicker = $("#" + fieldName).data("kendoDatePicker");
                            datepicker.value(appetizer.tools.getDate(cellValue));
                        } 
                        else if (isRadioButton() == "radio") {
                            $("input[name=" + nodeName + "][value=" + cellValue + "]").attr('checked', 'checked');
                            ...
                            //$('#' + fieldName).val(cellValue);
                            //$('#l' + fieldName).html(cellValue);
                            $("#" + primaryKeyTag).find('#' + fieldName).val(cellValue);
                            $("#" + primaryKeyTag).find('#l' + fieldName).html(cellValue);
                        }
                    }
                }
            }
...
CheckValidTime: function (key) {
    var input = $('#' + key).val();
    //debugger;
    if (!/^(0?[1-9]|1[012])(:[0-5]\d) [APap][mM]$/.test(input)) {
        $('#' + key).val('');
        $('#e' + key).html('Enter valid time');
        $('#e' + key).addClass('showError');
        $('#e' + key).removeClass('hideError');
        appetizer.message.showError('#showErrorMsg', 'Enter valid time!', 10000);
        return;
    }

    var parts = input.split(':');
    if (parts[0] > 12 || parts[1] > 59 ) {
        $('#' + key).val('');
        $('#e' + key).html('Enter valid time');
        $('#e' + key).addClass('showError');
        $('#e' + key).removeClass('hideError');
...
CheckMinMax: function (key,min,max) {
    try {
        var input = $('#' + key).val();
        //debugger;
        if (!/^(0?[0-9]\d*)$/.test(input)) {
            $('#' + key).val('');
            $('#e' + key).html('Enter a positive number');
            $('#e' + key).addClass('showError');
            $('#e' + key).removeClass('hideError');
        ...
        if (value < min || value > max) {
            ...
            $('#' + key).val('');
            $('#e' + key).html('Min value:'+min+' and Max value:'+max);
            $('#e' + key).addClass('showError');
            $('#e' + key).removeClass('hideError');
            appetizer.message.showError('#showErrorMsg', 'Min value:' + min + ' and Max value:' + max, 10000);
            return;
        }
        else {
            $('#e' + key).removeClass('showError');
...
//Added By Ashiqur Rahman Emran
//Added Date 17-09-2018
//Date Handling Items Start
date: {
    //Compare Start Date to End Date
        comparison: function (startDate, endDate) {
            try {
        ...
        fromUnixTimeStamp: function (UNIX_TimeStamp, isName , dividor ) {
            if (UNIX_TimeStamp == null) return '';
            var a = new Date(parseInt(UNIX_TimeStamp.match(/\d+/g)[0]));
            var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
            var year = a.getFullYear();
            var month = (isName ? months[a.getMonth()] : a.getMonth());
            var date = a.getDate();
            var date = date + dividor + month + dividor + year;
            return date;
        }
}
//Date Handling Items End
};
...
var SJYesNo = ["Please Select","Yes","No","Not Applicable"];
//Array Protyping
// Added By Ashiqur Rahman Emran
//This function with pick the index from item array and it will return these with comma
Array.prototype.concatWithIndex = function (item, spilittedChar) {

    if (item === null || item === '' || typeof item === 'undefined') {
        return '';
    }
    if (spilittedChar.length > 1)
        throw 'Splitted Char length can not be greater than 1 concatWithIndex';
    var result = '';
    var indexArray = String(item).split(spilittedChar);
    for (var i = 0; i < indexArray.length; i++) {
        result += this[indexArray[i]] + ',';
    }
