...
jGrid.prototype.init = function (isSingle) {
    ...
    var jhtmlMulti = '<div id="' + this.jgridBase + '"><div class="card-style">' +
        ...
        '         <div class="baseDiv"> <div id="contentMaster' + this.jgridBase + '"  style="padding:5px;">' +
        this.masterInner +
        '           </div>' +
        ...
        '               <span class="export" id="btnMap' + this.jgridBase + '" onclick="showMapProxy(' + this.jgridBase + ')" style="right: 3px;">Map</span>' +
        ...
        '                       <div style="width:50%;display:inline-block;">' +
        '                            <div class="btn-group dropup" style="width:105%;">' +
        '                                <button type="button" class="btn btn-success" data-toggle="dropdown" style="width:100%;height:31px;padding: 0;font-weight:bold;">' +
        '                                    Verify' +
        '                                </button>' +
        '                                <ul class="dropdown-menu" style="min-width: 100%;">' +
        '                                    <li><a id="1" class="linkButton" onClick="onRecordVerifyProxy(\'' + this.jgridBase + '\',this.id)">Verify Selected</a></li>' +
...
jGrid.prototype.setSlimScroll = function (modelName) {
    if (modelName.length > 0) {
        $('#' + modelName).slimScroll({
            height: (this.gridHeight - 69) + 'px'
        });
    }
    $('#content' + this.jgridBase).slimScroll({
        height: (this.gridHeight - 64) + 'px'
    });
        ...
        for (var key in data[i]) {
            if (key != "") {
                ...
                if (fieldName == this.masterRecordKeyField) {
                    ...
                    $(e).attr('id', cellValue);
                    $(d).attr('id', cellValue);
                    $(e).click(function (event) { op.onRecordEdit(event, this.id, modelName, true); });
                    $(d).click(function (event) { op.onRecordDelete(event, this.id, modelName, true, op.onMasterItemClick); });
...
function onRecordEditProxy(e, base, id, modelName, isMaster) {
    globalObjectPointer[base.id].onRecordEdit(e, id, modelName, isMaster) /// globalObjectMap
}
...
// Invoke edit modal
jGrid.prototype.onRecordEdit = function (e, id, modelName, isMaster) {
    e.preventDefault();
    ...
    appetizer.actionCall(this.actionURL, parm, "GET", function (data, result) {
        if (result) {
            ...
            //}
            $emodal.modal("show");
            $emodal.find("div.modal-body").css("pointer-events", "none");
            //setTimeout(function () { appetizer.div.setJsonObjectToDiv(data.json, "modalContent"); }, 10000);
            appetizer.div.setJsonObjectToDiv(data.json, "modalContent");
            $emodal.find("div.modal-body").css("pointer-events", "all");
...
function onRecordDeleteProxy(event,base, id, modelName, isMaster, notifyGrid) {
    globalObjectPointer[base.id].onRecordDelete(event,id, modelName, isMaster, globalObjectPointer[base.id].onMasterItemClick); /// globalObjectMap
}

// Perform delete operation
jGrid.prototype.onRecordDelete = function (e, id, modelName, isMaster, notifyGrid) {
    e.preventDefault();
...
jGrid.prototype.addDataToTable = function (data, tbody, tr, col, modelName) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (this.isVerifiable[modelName] || this.isEditable[modelName] || this.isDeletable[modelName]) {
            ...
            if (this.isEditable[modelName]) manage += '<i style="margin-right: 4px;" class="hi-icon fa fa-pencil icon-fa "><a  href="#" onclick="onRecordEditProxy(event,' + this.jgridBase + ',' + data[i][col[0]] + ',\'' + modelName + '\', false)"></a></i>';
            if (this.isDeletable[modelName]) manage += '<i  class="hi-icon fa fa-trash icon-fa "><a  href="#" onclick="onRecordDeleteProxy(event,' + this.jgridBase + ',' + data[i][col[0]] + ',\'' + modelName + '\', false, this.onMasterItemClick)"></a></i>';
            manage += '</span>';
...
jGrid.prototype.showVerifyInfo = function (modelName, recordId) {//show need to modify
    ...
    appetizer.actionCall(this.action, parm, "GET", function (data, result) {
        if (result) {
            ...
            if (serverReturn.UserName != '' && typeof (serverReturn.UserName) != 'undefined' && serverReturn.UserName != null) {
                var name, names = serverReturn.UserName.split(" ");
                names.length > 1 ? name = names[1] : name = names[0]; // may be we can check mohammad, lenth, title for better name ??
                $("#lblCreated").html('Created by <span>' + name + '<span> on ' + getDate(serverReturn.DataCollectionDate) + '</span>');
            }
            else {
                $("#lblCreated").html('User id [Not Set]');
            }
            if (serverReturn.VerifierName != '') {
                $("#lblVerified").html('Verified By <span>' + (serverReturn.VerifierName || ' [Not Set] ') + '<span> on ' + (getDate(serverReturn.VerificationDate)||' [Not Set] ') + '</span>');
            }
            else {
                $("#lblVerified").html('Record not verified');
            }

            if (serverReturn.IsVerified == 0 || serverReturn.IsVerified == null) {
                $("#lblStatus").html('Pending');
            }
            else if (serverReturn.IsVerified == 1) {
                $("#lblStatus").html('Approved');
            }
            else if (serverReturn.IsVerified == 2) {
                $("#lblStatus").html('Invalid data');
            }
            else if (serverReturn.IsVerified == 3) {
            ...
            $('#VerificationNote').val(serverReturn.VerificationNote);
        }
        else {
            appetizer.message.showError('#showErrorMsg', data.Data, 10000);
        }
...
jGrid.prototype.onDataExport = function (model, extension) {
    ...
    this.filter.RequestType = 'Export';
    var recursiveEncoded = $.param(this.filter);
    
    
    var urlpath = this.actionURL + '?' + recursiveEncoded;
