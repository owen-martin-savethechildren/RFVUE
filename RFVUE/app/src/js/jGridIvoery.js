...
function jGrid(base, caption, action, filter, theme) {
    this.action = action;
    this.filter = filter;
    this.jgridBase = base;
...
jGrid.prototype.updateBaseHtml = function (value) {
    this.updateBaseHtml = value;
}
...
jGrid.prototype.init = function (isSingle) {
    ...
    for (var i = 0; i < this.updateBaseHtml.length; i++) {
        if (this.updateBaseHtml[0] !== undefined) {
            var tok = this.updateBaseHtml[i].split(':');
            $('#' + tok[0]).html(tok[1]);
        }
    }
    ...
    this.gridHeight = screen.height - 300;

    if (typeof this.theme === 'undefined' || this.theme == null || this.theme == '') this.theme = 'Ivory';
    eval('var jHtml = isSingle ? this.GetTheme' + this.theme + '(0) : this.GetTheme' + this.theme + '(1)');
    //$jgrid.replaceWith(isSingle ? jhtmlSingle : jhtmlMulti);
    $jgrid.siblings().remove(); // Because JGid header & save export toolbar added dynamically every time adjecent befor the jgrid base 
    $jgrid.replaceWith(jHtml);
    ...
    $($jgrid).on('keydown', function (e) {
        ...
        switch (e.which) {
            case 37: // left
                if (this.masterCurrentPage > 0) container.jspagination('go', this.masterCurrentPage - 1);
                break;
            case 38: // up
...
jGrid.prototype.setSlimScroll = function (modelName) {
    if (modelName.length > 0) {
        $('#' + modelName).slimScroll({
            height: (this.gridHeight - (59 + 1)) + 'px'
        });
    }
    $('#content' + this.jgridBase).slimScroll({
        height: (this.gridHeight - (54 + 1)) + 'px'
    });
...
jGrid.prototype.createPaging = function (op, isMaster, base, totalRecord) {
    ...
    var container = $('#' + base).jspagination({
    ...
    container.jspagination('go', this.masterCurrentPage);
...
jGrid.prototype.bindMaster = function (modelName, templateId, callbackFn, isVerifyMaster) {
    ...
    var exportModel = modelName.replace("View", "");
    var extension = "";
    exportModel = exportModel.toUpperCase() === exportModel ? exportModel : exportModel.match(/[A-Z][a-z]+|[0-9]+/g).join(" ");
    var exportHtml = '<a onclick="onDataExportProxy(\'' + modelName + '\'' + '\,' + '\'' + this.jgridBase + '\'' + '\,' + '\'' + extension + '\')">' + exportModel + '</a>';//'<a onclick="onDataExportProxy(\'' + modelName + '\'' + '\,' + '\'' + this.jgridBase + '\')">' + exportModel + '</a>';

    $('#ExportDiv' + this.jgridBase).html(exportHtml);
}

jGrid.prototype.bindExport = function (value) {
    var exportHtml = '';
    if (value != undefined && value != null) {
        for (var i = 0; i < value.length; i++) {
            //var tok = value[i].split(':');
            //if (tok.length > 1)
            exportHtml += '<a onclick="onDataExportProxy(\'' + value[i].ModelName + '\'' + '\,' + '\'' + this.jgridBase + '\'' + '\,' + '\'' + value[i].Extension + '\')">' + value[i].DisplayName + '</a>';
        }
        $('#ExportDiv' + this.jgridBase).html(exportHtml);
    }
    else {
        console.error("bindExport value is undefined!");
        $('#ExportDiv' + this.jgridBase).html(exportHtml);
    }

}

jGrid.prototype.bindExportRF = function (value) {
    var html = '';
    if (value != undefined && value != null) {
        for (var i = 0; i < value.length; i++) {
            var aClick = 'onclick="onDataExportProxy(\'' + value[i].ModelName + '\'' + '\,' + '\'' + this.jgridBase + '\'' + '\,' + '\'' + value[i].Extension + '\')"';
            html += '<li class="kt-nav__item">' +
                '<a ' + aClick + ' class="kt-nav__link">' +
                '<i class="kt-nav__link-icon flaticon2-file"></i>' +
                '<span class="kt-nav__link-text">' + value[i].DisplayName + '</span>' +
                '</a>' +
                ' </li>';
        }
        $('#rf-jgrid-data-export-list').empty();
        $('#rf-jgrid-data-export-list').append(html);
    }
    else {
        console.error("bindExport value is undefined!");
        $('#rf-jgrid-data-export-list').append('');
    }
}

jGrid.prototype.bindMapRF = function (value) {
    var html = '';
    if (value != undefined && value != null) {
        for (var i = 0; i < value.length; i++) {
            var aClick = 'onclick="onDataExportProxy(\'' + value[i].ModelName + '\')"';
            html += '<li class="kt-nav__item">' +
                '<a ' + aClick + ' class="kt-nav__link">' +
        ...
        $('#rf-jgrid-map-list').empty();
        $('#rf-jgrid-map-list').append(html);
    }
    else {
        console.error("bindExport value is undefined!");
        $('#rf-jgrid-map-list').append('');
    }
...
jGrid.prototype.onRecordEditUpdate = function (notifyGrid) {
    ...
    var data = appetizer.div.getJsonObjectOnlyFromDiv("editModal", '0');

    if (this.filter['OverrideModel'] !== undefined) data["OverrideModel"] = this.filter['OverrideModel'];
    ...
    var parm = {
        "ModelName": this.getEntityModel(this.currentModelName), "RequestType": "Update", "RecordType":
            this.currentModelType, "RecordId": recordId, "IsOverride": true, "Data": JSON.stringify(data)
    };
...
jGrid.prototype.onRecordEdit = function (id, modelName, isMaster) {
    ...
    var parm = {
        "ControllerName": this.masterModelName, "ModelName": this.getEntityModel(modelName),
        "RequestType": "ShowEdit", "RecordType": recordType, "IsOverride": true, "RecordId": id, "Data": JSON.stringify(this.filter)
    };
    ...
    appetizer.actionCall(this.actionURL, parm, "GET", function (data, result) {
        if (result) {
            ...
            var title = '';
            modelName == 'QB' ? title = 'MonitoringChecklist' : title = modelName;
            $("#modalTitle").text(title.match(/[A-Z][a-z]+|[0-9]+/g).join(" ") + ' Information');
            ...
            //    localStorage.setItem("editData", JSON.stringify(data.json));
            //}
            //else {
            //    console.error("Sorry, your browser does not support Web Storage...");
            //}
            $emodal.modal("show");
            setTimeout(function () { appetizer.div.setJsonObjectToDiv(data.json, "modalContent"); }, 5000);


            $emodal.on('hidden.bs.modal', function () {         /// Added by SHOHID
                $emodal.find("div.modal-body").html("");
            });
        }
        else {
            appetizer.ajaxLoder.hidePleaseWait();
                ...
                if (result) {
                    if (isMaster) {
                        ...
                        if (notifyGrid != null) notifyGrid(op.currentMasterRecordId);
                            ...
                            // parm = { "ModelName": op.currentModelName == "" ? op.masterModelName : op.currentModelName, "RequestType": "Show", "RecordType": "Detail", "RecordId": op.currentMasterRecordId, "PageSize": op.pagingSize, "PageNo": 1, "Data": JSON.stringify(op.filter) };
                            parm = { "ModelName": op.currentModelName == "" ? op.masterModelName : op.currentModelName, "RequestType": "Show", "RecordType": "Detail", "RecordId": op.currentMasterRecordId, "PageSize": op.pagingSize, "PageNo": 1, "Data": JSON.stringify(op.filter) };
                            op.callDetailAction(op.currentModelName == "" ? op.masterModelName : op.currentModelName, parm, op.jgridBase);
                        }
                    }
                    //notifyGrid(op.currentMasterRecordId);
...
jGrid.prototype.bindDetails = function (detailObject, data, isVerify, isEdit, isDelete, isFlex, isList, columnWidths, columnNames) {
    ...
    var parm = { "ModelName": detailObject, "RequestType": "Show", "RecordType": "Detail", "RecordId": data, "Data": JSON.stringify(this.filter) };
...
function addAllColumnHeaders(myList, selector) {
    ...
    for (var key in rowHash) {
        if ($.inArray(key, columnSet) == -1) {
            columnSet.push(key);
            if (key == 'id') headerTr$.append($('<th style="display:none">').html(key)); else headerTr$.append($('<th style="background:#ebebeb;width:' + width[widthPos] + '%;">').html(key));
            widthPos++;
        }
    }
    headerTr$.append($('<th style="background:#ebebeb;width:10%;">').html('Result'));
    headerTr$.append($('<th style="background:#ebebeb;width:30%;">').html('Remarks'));
    //}
    $('#' + selector).append(headerTr$);
    return columnSet;
}
jGrid.prototype.bindCascadeForm = function (baseForm, modelName, cascadeModelName, data) {
    this.currentMasterRecordId = data;
    var parm = { "ModelName": modelName, "RequestType": "Show", "RecordType": "Detail", "RecordId": data, "Data": JSON.stringify(this.filter) };
    var op = this;
    appetizer.actionCall(this.action, parm, "GET", function (data, result) {
        if (result) {
            var serverReturn = data.Data.Record;
            $('#' + baseForm).html('<table id="' + modelName + '" class="table table-bordered table-striped table-hover"></table >');
            var columns = addAllColumnHeaders(serverReturn, modelName);
            for (var i = 0; i < serverReturn.length; i++) {
                var SL = '';
                var row$ = $('<tr/>');
                for (var colIndex = 0; colIndex < columns.length; colIndex++) {
                    var cellValue = serverReturn[i][columns[colIndex]];
                    if (colIndex == 1) SL = cellValue;
                    if (cellValue == null) { cellValue = ""; }
                    if (colIndex == 0) row$.append($('<td style="display:none">').html(cellValue));
                    else if (colIndex == 1) row$.append($('<td style="font-weight: 500;">').html(cellValue.replace('_', '.')));
                    else row$.append($('<td style="text-align:left;font-weight: 500;">').html(cellValue));
                }
                row$.append($('<td style="font-weight: 500;">').html($('<span id="Q' + SL + '">').html(i)));
                row$.append($('<td style="font-weight: 500;">').append($('<span id="Q' + SL + '_Rem1">').html('')).append(' ').append($('<span id="Q' + SL + '_Rem2">').html('')));
                $('#' + modelName).append(row$);
            }
            parm = { "ModelName": cascadeModelName, "RequestType": "Show", "RecordType": "Detail", "IsOverride": true, "RecordId": op.currentMasterRecordId, "Data": JSON.stringify(op.filter) };
            appetizer.actionCall(op.action, parm, "GET", function (data, result) {
                if (result) {
                    var serverReturn = data.Data.Record[0];
                    //appetizer.div.setJsonObjectToDiv(serverReturn, baseForm);
                    var rem1 = '';
                    var key1 = '';
                    Object.keys(serverReturn).forEach(function (key) {

                        if (key1 == key.split("m1").join("m").split("m2").join("m") && (rem1 != "" && serverReturn[key] != ""))
                            $("#" + baseForm).find('#' + key).html(" and " + serverReturn[key]);
                        else
                            $("#" + baseForm).find('#' + key).html(serverReturn[key]);
                        rem1 = serverReturn[key];
                        key1 = key.split("m1").join("m").split("m2").join("m");
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        for (var j = 0; j < col.length; j++) {
            ...
            if (!col[j].includes('Color')) {
                ...
                // var f =document.getElementById(fld + i);
                if (data[i][col[j]] != "#fff")
                    $('#' + fld + i).css("background-color", data[i][col[j]]);
            }
...
jGrid.prototype.auto = function (modelName, data) {
    ...
    var tmpColorCol = []; // all color columns
    var col = [];
    for (var i = 0; i < data.length; i++) {
        for (var key in data[i]) {
            if (col.indexOf(key) === -1) {
                if (!key.includes('Color')) col.push(key);
                else tmpColorCol.push(key);
            }
        }
    }
    ...
    for (var i = 0; i < tmpColorCol.length; i++) col.push(tmpColorCol[i]); // all color columns    
    ...
    appetizer.actionCall(this.actionURL, parm, "GET", function (data, result) {
        if (result) {
            ...
            if (singlePaging) {
                ...
                //op.createPaging(op, true, 'light-pagination-single' + gridBase, total);
            }
        }
        else {
            appetizer.message.showError('#showErrorMsg', data.Data, 10000);
...
jGrid.prototype.setVerificationInfo = function () { //discard - no need 
    ...
    if (this.selctedMasterJsonData.UserName != '' && this.selctedMasterJsonData.UserName != null && this.selctedMasterJsonData.UserName !== "undefined") {
...
function onDataExportProxy(model, base, extension) {
    globalObjectPointer[base].onDataExport(model, extension);
}

jGrid.prototype.onDataExport = function (model, extension) {
    this.filter.ModelName = model;
    this.filter.Extension = extension;
    var jsonString = JSON.stringify(this.filter);
    var urlpath = '/JGrid/Export?filterData=' + jsonString;
    window.open(urlpath, "_blank");

    //if (this.masterModelName == "SponsorDetails") {
    //    var urlpath = '/JGrid/Export?ModelName=' + this.masterModelName + "Photo";
    //    window.open(urlpath, "_blank");
    //}


    ////var parm = { "ModelName": this.masterModelName, "RequestType": "Export", "RecordType": "Detail", "Data": JSON.stringify(this.filter) };
    //////var parm = { "ControllerName": this.masterModelName, "ModelName": this.getEntityModel(modelName), "RequestType": "ShowCreate", "RecordType": recordType };
    ////appetizer.actionCall('/JGrid/Export', parm, "GET", function (data, result) {
    ////    if (result) {
    ...
    ////    }
    ////});
    ////var urlpath = '/JGrid/Export?filter=' + this.masterModelName;
...
jGrid.prototype.showMap = function () {
    //var parm = { "ModelName": this.getEntityModel(this.currentModelName), "RequestType": "Map", "RecordType": "Detail", "Data": JSON.stringify(this.filter) }; // Commented by Shohid Because this.CurrentModelName is null
    var parm = { "ModelName": this.getEntityModel(this.masterModelName), "RequestType": "Map", "RecordType": "Detail", "Data": JSON.stringify(this.filter) };
...
jGrid.prototype.addCreateButton = function (createModelName) {
    this.createModel = createModelName;
    $('#btnCreateNew' + this.jgridBase).removeClass('hidden');
    $('#btnMap' + this.jgridBase).css('margin-right', '30px');
}

function showCreateNewProxy(base) {
    globalObjectPointer[base.id].onRecordCreate(globalObjectPointer[base.id].createModel, false) /// globalObjectMap
}


// Invoke create modal
jGrid.prototype.onRecordCreate = function (modelName, isMaster) {
    var recordType = isMaster ? 'Master' : 'Detail';
    var parm = { "ControllerName": this.masterModelName, "ModelName": this.getEntityModel(modelName), "RequestType": "ShowCreate", "RecordType": recordType };
    var op = this;
    appetizer.actionCall(this.actionURL, parm, "GET", function (data, result) {
        if (result) {
            appetizer.ajaxLoder.hidePleaseWait();
            $emodal = $("#createModal");
            $("#createModal #modalTitle").text(modelName.match(/[A-Z][a-z]+|[0-9]+/g).join(" ") + ' Entry');
            let frag = document.createRange().createContextualFragment(data.html);
            $emodal.find("div.modal-body").html(frag);
            isMaster ? $('#createDialogBase').css('width', op.masterModalWidth) : $('#createDialogBase').css('width', op.detailModalWidth);
            $('#createModal :input').removeAttr('readonly');
            $emodal.modal("show");
...
function onRecordCreateProxy(base) {
    globalObjectPointer[base.id].onRecordCreateSave(globalObjectPointer[base.id].onMasterItemClick, globalObjectPointer[base.id].createModel)
}

// Invoke after edit save
jGrid.prototype.onRecordCreateSave = function (notifyGrid, modelName) {
    //var data = appetizer.div.getJsonObjectFromDiv("createModal", '0');
    var data = appetizer.div.getJsonObjectOnlyFromDiv("createModal", '0');
    //console.log(data);
    var parm = { "ModelName": this.getEntityModel(modelName), "RequestType": "Insert", "Data": JSON.stringify(data) };
    ...
    appetizer.actionCall(this.actionURL, parm, "GET", function (data, result) {
        if (result) {
            ...
            //    }
            //}
            appetizer.message.showInfo('#showMsg', 'Record saved successfully', 2500);
        }
        else {
            appetizer.ajaxLoder.hidePleaseWait();
            appetizer.message.showError('#showErrorMsg', data.Data, 10000);
...
// has removed to init function
//$(this.base).keydown(function (e) {
//    //if not single //scroll prob
//    var currentDiv = $(".j-grid-master-active").parent();
//    var nextDiv = currentDiv.next();
//    var prevDiv = currentDiv.prev();
...
function sendPushNotification() {
    appetizer.message.showConfirm("Do you want to send notification?", function (data) {
        if (data) {
            appetizer.ajaxLoder.showPleaseWait();
            var urlpath = '/JGrid/SendNotification';
            appetizer.actionCall(urlpath, null, 'GET', function (data, result) {
                if (result) {
                    appetizer.ajaxLoder.hidePleaseWait();
                    appetizer.message.showInfo("#showMsg", "Notification send successful!", 2500);
                }
                else {
                    appetizer.ajaxLoder.hidePleaseWait();
                    appetizer.message.showError('#showErrorMsg', data.Data, 10000);
...
jGrid.prototype.GetThemeBasic = function (type) {
    ...
    var jhtmlSingle = '<div id="' + this.jgridBase + '"><div class="card-style">' +
        ...
        '               <div class="j-grid-detail-caption" id="DetailsCaption1" style="background-color: #ededed;border-top-left-radius: 4px;border-top-right-radius: 4px;">' + this.gridCaption + '</div>' +
    ...
    var jhtmlMulti = '<div id="' + this.jgridBase + '"><div class="card-style">' +
        '<div class="containerJGird" style="">' +
        ...
        '             <div class="j-grid-master-caption">' + this.gridCaption + '<div class="dropdown pull-right"><span>Export </span><img class="dropbtn" style="width:7px;" src="../Images/arrow-down.png"><div class="dropdown-content" id="ExportDiv' + this.jgridBase + '"></div></div></div>' +
        //'             <span class="export" onclick="onDataExportProxy(' + this.jgridBase + ',1)">Export </span>' +
        '         <div class="baseDiv"> <div id="contentMaster' + this.jgridBase + '"  style="padding:5px;width:462px;">' +
        this.masterInner +
        '           </div>' +
        ...
        '       <div class="col-md-8" style="padding:0;float:left;padding-left:0px;">' +
        '           <div class="j-grid-detail-layout">' +
        '               <div class="j-grid-detail-caption" id="DetailsCaption1"></div>' +
        '               <span class="export" id="btnMap' + this.jgridBase + '" onclick="showMapProxy(' + this.jgridBase + ')" style="display:none; right: 3px;">Map</span>' +
        '               <span class="btn btn-success btnAddNew hidden" id="btnCreateNew' + this.jgridBase + '" onclick="showCreateNewProxy(' + this.jgridBase + ')" style="right: 3px;">+</span>' +
        '               <div id="content' + this.jgridBase + '" style="padding:5px;">' +
        this.detailInner +
        '               </div>' +
        '               <div class="j-grid-detail-base">' +
        '                   <div class="col-md-2" style="padding:0;float:left;" >' +
        '                       <div class="j-grid-created-by"><span id="lblCreated"></span></div>' +
        '                   </div>' +
        '                   <div class="col-md-2" style="padding:0; float:left;">' +
        '                       <div class="j-grid-verified-by"><span id="lblVerified"></span></div>' +
        '                   </div>' +
        '                   <div class="col-md-5" style="padding:0;margin-top:-1px; float:left;">' +
        '                       <div style="padding-right:5px;padding-left:5px;"><input type="text" id="VerificationNote" placeholder="Verification Note" style="font-size:13px;width:100%;height:33px;" class="k-textbox" /></div>' +
        '                   </div>' +
        '                   <div class="col-md-3" style="padding:0;margin-top:-1px; float:left;">' +
        '                        <div style="padding-right:0px;padding-left:5px; width:45%;display:inline-block;vertical-align:top;text-align:center;">' +
        '                            <div class="j-grid-created-by"><span>Status<br /><span id="lblStatus"></span></span></div>' +
        '                        </div>' +
        '                       <div style="width:50%;display:inline-block;">' +
    ...
    return type == 0 ? jhtmlSingle + this.GetCommonTemplate() : jhtmlMulti + this.GetCommonTemplate();
}

jGrid.prototype.GetThemeIvory = function (type) {
    var toolbar = $("#rf-templates").find('#jGridToolbar').html();
    //console.log(toolbar);
    var jhtmlSingle = toolbar + '<div id="' + this.jgridBase + '"><div class="card-style">' +
        //'<div class="col-md-12" id="j-loader" style="width:100%;height:100%;border-radius: 5px;"><div id="spinner"><img alt="" src="/images/j-loader.gif"></div></div>' +
        '        <div class="col-md-12" style="padding:0;">' +
        '           <div class="j-grid-detail-layout" style="border: #ededed solid 2px;">' +
        '               <div class="j-grid-detail-caption" id="DetailsCaption1" style="background-color: #ededed;border-top-left-radius: 4px;border-top-right-radius: 4px;">' + this.gridCaption + '</div>' +
        '               <div id="content' + this.jgridBase + '" style="padding-top:5px; padding-bottom:5px;background-color:#EDEDED">' +
        this.detailInner +
        '               </div>' +

        '              <div id="pagingBase">' +
        '              <div class="pagination-holder j-grid-master-paging" style="bottom:4px;width: 40%;border-bottom-left-radius: 4px;border-bottom-right-radius: 4px;">' +
        '                   <div style="display:inline-block;width:68%;">' +
        '                        <div id="light-pagination-single' + this.jgridBase + '" class="pagination light-theme simple-pagination">' +
        '                        </div>' +
        '                   </div>' +
        ...
        '                            <div class="j-grid-created-by"><span>Status<br /><span id="lblStatus"></span></span></div>' +
        '                        </div>' +
        '                       <div style="width:50%;display:inline-block;margin-top:-3px;">' +
        '                            <div class="btn-group dropup" style="width:105%;">' +
        '                                <button type="button" class="btn btn-success" data-toggle="dropdown" style="width:100%;background-color: rgba(29, 201, 183, .4);color: #5f5454;">' +
        '                                    Verify' +
        '                                </button>' +
    ...
    var jhtmlMulti = toolbar + '<div id="' + this.jgridBase + '"><div class="card-style">' +
        '<div class="containerJGird" style="background-color:#EDEDED">' +
        //'<div class="col-md-12" id="j-loader"><div id="spinner"><img alt="" src="/images/j-loader.gif"></div></div>' +
        //'<div id="preloader"><div id="spinner"><img alt="" src="images/preloaders/4.gif"></div></div>'+
        '<div class="col-md-4" style="padding:0;float:left;">' +
        '           <div class="j-grid-master-layout">' +
        '             <div class="j-grid-master-caption">' + this.gridCaption + '<div class="dropdown pull-right"><span>Export </span><img class="dropbtn" style="width:7px;" src="../Images/arrow-down.png"><div class="dropdown-content" id="ExportDiv' + this.jgridBase + '"></div></div></div>' +
        //'             <span class="export" onclick="onDataExportProxy(' + this.jgridBase + ',1)">Export </span>' +
        '         <div class="baseDiv"> <div id="contentMaster' + this.jgridBase + '"  style="padding-top:5px;padding-left:0;">' +
        this.masterInner +
        '           </div>' +
        '              <div class="pagination-holder j-grid-master-paging">' +
        '                   <div style="display:inline-block;width:68%;">' +
        ...
        '               <span class="export" id="btnMap' + this.jgridBase + '" onclick="showMapProxy(' + this.jgridBase + ')" style="display:none; right: 3px;">Map</span>' +
        '               <span class="btn btn-success btnAddNew hidden" id="btnCreateNew' + this.jgridBase + '" onclick="showCreateNewProxy(' + this.jgridBase + ')" style="right: 3px;">+</span>' +
        '               <div id="content' + this.jgridBase + '" style="padding:0px  ;margin-top: 4px;">' +
        this.detailInner +
        '               </div>' +
        '               <div class="j-grid-detail-base">' +
        '                   <div class="col-md-2" style="padding:0;float:left;" >' +
        ...
        '                       <div class="j-grid-verified-by"><span id="lblVerified"></span></div>' +
        '                   </div>' +
        '                   <div class="col-md-5" style="padding:0;margin-top:-6px; float:left;">' +
        '                       <div style="padding-right:5px;padding-left:5px;"><input type="text" id="VerificationNote" placeholder="Verification Note" style="font-size:13px;width:100%;height:33px;" class="k-textbox" /></div>' +
        '                   </div>' +
        '                   <div class="col-md-3" style="padding:0;margin-top:-6px; float:left; padding-right: 5px;">' +
        '                        <div style="padding-right:0px;padding-left:5px; width:45%;display:inline-block;vertical-align:top;text-align:center;">' +
        '                            <div class="j-grid-created-by" style="margin-top: -4px;"><span>Status<br /><span id="lblStatus"></span></span></div>' +
        '                        </div>' +
        '                       <div style="width:50%;display:inline-block;margin-top: -3px;">' +
        '                            <div class="btn-group dropup" style="width:105%;">' +
        '                                <button type="button" class="btn btn-success" data-toggle="dropdown" style="width:100%;background-color: rgba(29, 201, 183, .4);color: #5f5454;">' +
        '                                    Verify' +
        '                                </button>' +
        '                                <ul class="dropdown-menu" style="min-width: 100%;width: 150px;margin-left: -40px;">' +
        '                                    <li><a id="1" class="linkButton" style="font-weight: normal;" onClick="onRecordVerifyProxy(\'' + this.jgridBase + '\',this.id)">Verify Selected</a></li>' +
        '                                    <li><a id="2" class="linkButton" style="font-weight: normal;" onClick="onRecordVerifyProxy(\'' + this.jgridBase + '\',this.id)">Reject Selected</a></li>' +
        '                                    <li><a id="0" class="linkButton" style="font-weight: normal;" onClick="onRecordVerifyProxy(\'' + this.jgridBase + '\',this.id)">Reset Selected</a></li>' +
        '                                </ul>' +
        '                            </div>' +
        '                        </div>' +
        '                    </div>' +
...
jGrid.prototype.GetCommonTemplate = function () {
    var html =
        //Edit modal template
        ...
        '        </div>' +
        '    </div>' +
        //'<script>$("#editModal").on("shown.bs.modal", function () {debugger; appetizer.div.setJsonObjectToDiv(JSON.parse(localStorage.getItem("editData")), "modalContent")});</script>' +
        '</div>' +

        //New entry modal template
        '   <div class="modal fade" id="createModal">' +
        '    <div id="modalContent">' +
        '        <div id="createDialogBase" class="modal-dialog">' +
        '            <div class="modal-content">' +
        '                <div class="modal-header">' +
        '                    <button type="button" class="close"' +
        '                            data-dismiss="modal" aria-label="Close">' +
        ...
        '                <div class="modal-footer">' +
        '                    <button type="button" class="btn" data-dismiss="modal">Close</button>' +
        '                    <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="onRecordCreateProxy(' + this.jgridBase + ')">Save changes</button>' +
        '                </div>' +
        '            </div>' +
        '        </div>' +
        '    </div>' +
...
/*
   // OLD Single Html Modal Popup Tamplate
